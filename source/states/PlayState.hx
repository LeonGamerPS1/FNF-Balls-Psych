...
class PlayState extends MusicBeatState
{
	...
	private function generateSong():Void
	{
		...
		for (section in sectionsData)
		{
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				for (tail in swagNote.tail)
					if (!isPixelStage && tail.animation.name.contains('end'))
					{
						tail.scale.y = 0.7;
						tail.extraData['end'] = true;
						tail.updateHitbox();
					}
	...
	function strumPlayAnim(isDad:Bool, id:Int, time:Float)
	{
		...
		{
			spr.playAnim('confirm', true);
			time = spr.animation.curAnim.numFrames / spr.animation.curAnim.frameRate;
			spr.resetAnim = time;
		}
	}
...
class AntiNoteOverlap extends FlxShader {
	@:glFragmentSource("
	// HaxeFlixel custom shader snippet (GLSL)
#pragma header

void main() {
    vec4 tex = flixel_texture2D(bitmap, openfl_TextureCoordv);
    // normal color * your alpha
    vec4 col = tex * openfl_Alphav;

    // force final alpha to remain the same (no stacking effect)
